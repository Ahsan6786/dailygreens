...
export function Footer() {
  return (
    <footer className="bg-white  py-16 md:py-24 px-6 md:px-12 border-t border-black/5  transition-colors duration-500">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 md:gap-16">
        {/* Brand */}
        <div className="space-y-8">
          <div className="flex items-center gap-3">
            <Image src="/logo.png" alt="Daily Greens" width={48} height={48} className=" invert" />
            <span className="text-3xl font-black text-slate-900  italic tracking-tighter">Daily Greens</span>
          </div>
          ...
          <div className="flex gap-4">
            {[Globe, Users, Mail].map((Icon, i) => (
              <a 
                ...
                className="w-12 h-12 rounded-2xl bg-slate-50  flex items-center justify-center text-slate-400 hover:text-green-600 hover:bg-white   transition-all shadow-sm"
              >
        ...
        <div>
          ...
          <ul className="space-y-4">
            {["Home", "Our Story", "The Menu", "Reviews", "Gallery"].map((link) => (
              <li key={link}>
                <a href="#" className="text-slate-600  hover:text-green-600 transition-colors font-bold text-sm">{link}</a>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <p className="text-slate-600  text-sm mb-6 leading-relaxed font-bold">Get healthy tips and secret discounts directly in your inbox.</p>
          <div className="relative group">
            <input 
              type="email" 
              placeholder="Your Email" 
              className="w-full bg-slate-50  border border-slate-200  pl-6 pr-16 py-4 rounded-2xl focus:outline-none focus:ring-2 focus:ring-green-600/50 transition-all font-bold text-slate-900  placeholder:text-slate-400"
            />
